...
- For certifications and capabilities: if a field (brcgs, organic, rail_carrier, etc.) is absent from a facility's data, that facility does NOT have it. Absence = No. Only list facilities where the field is explicitly present and true. Most facilities do NOT have BRCGS — only list the ones that explicitly show it.
- Never say "all facilities have X" or "all except Y" — enumerate only the confirmed ones.
